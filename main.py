...
def print_all_f3():
    ...
    for n in range(4):
        for i in range(30):
            list1: List[WebElement] = d.find_elements(By.XPATH,
                                                      "//*[@id='mainContent']/index-lobby/index-composition/index-weight/gridview-lib/div/div[2]/div/div/div[2]/table/tbody/tr")
            for l in list1:
                if l.text is None:
                    continue
...
def reload_f4():
    ...
    correct_page(d)
    now = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    with open(f".\\Data\\data_{now}.csv", 'w', newline='', encoding='UTF8') as file:
        fieldNames = ['Name', 'Symbol', "Security No", "ISIN", "Weight (%)", "Market Cap (NIS millions)",
                      "Base Price", "Closing Price (0.01 NIS)", "Ex"]
...
def correct_page(driver) -> str:
    ...
    time.sleep(1)
    driver.find_element(By.XPATH,
                        "//*[@id='mainContent']/index-lobby/index-composition/index-weight/gridview-lib/div/div[2]/div/div/div[2]/table/thead/tr[2]/td[6]/ul/li[2]/button").click()
    time.sleep(1)
